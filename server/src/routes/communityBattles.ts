...
router.get('/:battleId', requireAuth, async (req: Request, res: Response) => {
  try {
...
router.put('/:battleId', requireAuth, async (req: Request, res: Response) => {
  try {
    const { battleId } = req.params;
...
router.post('/:battleId/vote', requireAuth, async (req: Request, res: Response) => {
  try {
    const { battleId } = req.params;
...
router.delete('/:battleId', requireAuth, async (req: Request, res: Response) => {
  try {
    const { battleId } = req.params;
...
router.post('/cleanup-expired', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    for (const user of users) {
      if (!user.communityBattles || user.communityBattles.length === 0) continue;
